...
//             // Add click event listener to open modal for adding new task
...
//             addTaskBtn.onclick = null; // Remove event listener
//         }
...
// async function updateStatus(oldName, newName) {
//     const payload = {
//         old: oldName,
//         new: newName
//     };
...
//     try {
//         const response = await fetch(`${API_URL}/status/delete`, {
//             method: 'DELETE',
//             headers: getAuthHeaders(),
...
//     try {
//         const res = await fetch(`${API.statuses}/${encodeURIComponent(statusName)}`, {
//             method: "DELETE",
//             headers: getAuthHeaders()
//         });
...
//         alert(`Status "${statusName}" deleted successfully!`);
...
// //             ACTIVE_STATUS = status.name;
// //         }

// //         tab.dataset.statusKey = status.name;
...
// //         // Only allow rename if user has add_status permission
// //         if (hasPermission('add_status')) {
// //             tab.addEventListener("dblclick", () => {
// //                 enableStatusRename(tab, tab.dataset.statusKey);
// //             });
// //         }
...
// //         // When there's an add button:
// //         // 1. All regular tabs should have square corners (no right border radius)
// //         // 2. Add button should have rounded corners
// //         const allTabs = bar.querySelectorAll('.tab:not(.add-tab)');
// //         allTabs.forEach(tab => {
// //             tab.style.borderRadius = '0';
// //         });

// //         // Add button gets rounded corners
// //         addBtn.style.borderRadius = "0rem 1.5rem 1.5rem 0rem";

// //     } else {
// //         // If no add button (manager/dev users), last status tab gets rounded corners
...
// //     // Reset first tab's left border radius
// //     if (STATUSES.length > 0) {
...
//             // Style the delete button with slide animation
//             Object.assign(delBtn.style, {
//                 position: "absolute",
//                 right: "-2rem",
//                 top: "50%",
...
//                     bar.appendChild(tab);
//                 } else {
//                     bar.insertBefore(tab, afterElement);
//                 }
//             });
...
//                 // Prevent rename when clicking delete button
...
//         // Reload statuses from backend to reset
//         await fetchStatus();
//         renderStatuses();
//     }
// }

// function enableStatusRename(tabEl, oldStatusKey) {
...
//         if (!newName || newName.toLowerCase() === oldStatusKey) {
//             tabEl.textContent = oldStatusKey;
//             return;
//         }

//         const newStatusKey = newName;

//         try {
//             await updateStatus(oldStatusKey, newStatusKey);

//             tabEl.dataset.statusKey = newStatusKey;

//             const status = STATUSES.find(s => s.name === oldStatusKey);
//             if (status) {
//                 status.name = newStatusKey;
//             }

//             if (ACTIVE_STATUS === oldStatusKey) {
//                 ACTIVE_STATUS = newStatusKey;
//             }
...
//     console.log("Pressed a status = ", status_id)
//     if (!status_id) return [];
...
// async function renderTasksForStatus(statusName = null) {
//     console.log("status_name here = ", statusName)
//     if (isRendering) {
//         return;
//     }
...
//         // Control the add task container visibility
//         const addTaskContainer = document.querySelector('.add-task-container');
//         if (addTaskContainer) {
//             addTaskContainer.style.display = hasPermission('add_task') ? 'flex' : 'none';
//         }

//         const tasks = await fetchTasks(statusName);

//         if (!tasks || tasks.length === 0) {
//             const empty = document.createElement("div");
//             empty.className = "task";
//             empty.innerHTML = statusName
//                 ? `<span class="task-header">No tasks in "${statusName}"</span>`
...
//             const statusMeta = STATUSES.find(s => s.name.toLowerCase() === normalizedStatus);
...
//             // Make status chip clickable for users with change_status permission
...
//             // Only show Edit button if user has edit_task permission
...
//             // Only show Delete button if user has delete_task permission
...
//         // Populate status dropdown and set current status
//         populateStatusDropdown();

//         // Make sure status dropdown shows the correct status
...
//     // Disable status dropdown if user only has change_status but not edit_task
//     // When editing, change_status allows changing status even without edit_task
...
// function openStatusChangeModal(taskId, taskTitle, currentStatus) {
//     // Store task info
...
//     document.getElementById('current-status-display').value = currentStatus;

//     // Populate status dropdown
//     populateStatusDropdownForChange(currentStatus);
...
// // Function to populate status dropdown for status change
// function populateStatusDropdownForChange(currentStatus) {
//     const statusSelect = document.getElementById('new-status-select');
//     if (!statusSelect) return;

//     statusSelect.innerHTML = '';

//     // Add options from STATUSES array
//     STATUSES.forEach(status => {
//         const option = document.createElement('option');
//         option.value = status.name;
//         option.textContent = status.name;

//         // Add CSS class based on status name (optional)
//         option.className = `option-${status.name.replace(' ', '-')}`;

//         // Set style based on status color
//         // if (status.color) {
//         //     option.style.backgroundColor = getStatusColorHex(status.color);
//         // }

//         // Don't show current status as an option (optional)
//         // if (status.name !== currentStatus) {
//         statusSelect.appendChild(option);
//         // }
//     });

//     // Select the first option (or you can select a default like "In Progress")
...
//     const userConfirmed = confirm(`Change status of task "${currentStatusChangeTask.title}" from "${currentStatusChangeTask.currentStatus}" to "${newStatus}"?`);
//     if (!userConfirmed) {
//         return;
//     }
...
//         const result = await addStatus(name, color);
...
//         alert(result.message || `Status "${name}" added successfully!`);
